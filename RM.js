...
document.getElementsByClassName("production-status__input")[0].value = "` + radcode + `"
...
if (prostat.substr(2,4) == "Ihr ")
{
  ...
  let LieferungBis = prostat.substr(beginnDatum, prostat.indexOf("bei Ihrem") - beginnDatum)
  let dategetrennt = LieferungBis.split(".")
  let LieferDatum = new Date(dategetrennt[2], dategetrennt[1] - 1, dategetrennt[0])
  
  let status = htmlget.substr(htmlget.indexOf("\"status\": \"") + 11, 
  ...
  if (
      ...
      || (today.getMonth() === LieferDatum.getMonth() && today.getDay() > LieferDatum.getDay())
  ) {
  ...
  let daysleft =  parseInt((LieferDatum.getTime() - today.getTime())/convertInDays);
  let n = new Date()
  log("R&M " + model)
  ...
  let line3 = widget.addText("Stand: " + n.getHours() + ":" + ("0" + n.getMinutes()).slice(-2))
